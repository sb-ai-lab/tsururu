...
def main():
    ...
    for (
        ...
        if transformer is None:
            if not (transform_target is True):
                continue
        elif transformer == "StandardScalerTransformer":
            if transformer_regime is not None or (
                transform_features is False and transform_target is False
            ):
                continue
        else:
            if (
        ...
        try:
            ...
            current_pred_test.to_csv(
                os.path.join(args.results_path / f"{model_name}_pred_test.csv"), index=False
            )

            current_pred_val.to_csv(
                os.path.join(args.results_path / f"{model_name}_pred_val.csv"), index=False
            )
            ...
            metrics_test.to_csv(
                os.path.join(args.results_path / f"{model_name}_metrics_test.csv"), index=False
            )
            ...
            metrics_val.to_csv(
                os.path.join(args.results_path / f"{model_name}_metrics_val.csv"), index=False
            )
            print("Success!")
