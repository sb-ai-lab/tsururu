...
@memory.cache
def get_df_with_train_mean_and_std(dataset_path: Path) -> pd.DataFrame:
    """Get DataFrame with mean and std for each id in the train dataset.

    Args:
        dataset_path: path to the dataset CSV file.

    Returns:
        DataFrame with columns 'mean' and 'std' for each id.

    """
    train_dataset, _, _ = get_train_val_test_datasets(
        dataset_path, dataset_params, TRAIN_SIZE, TEST_SIZE, HISTORY
    )
    stat_df = train_dataset.data.groupby(train_dataset.data[train_dataset.id_column])[
...
def read_old_metrics_get_times(metrics_path: Path) -> tuple:
    """Reads the metrics file to extract fit_time and forecast_time.

    Args:
        metrics_path: path to the metrics CSV file.

    Returns:
        (fit_time, forecast_time) or (None, None) if
            the file does not exist or the values are not found.
...
def process_file(
    ...
    """Save MAE and RMSE metrics for each id with normalization.

    Args:
        model_pred_path: path to the model predictions CSV file.
        old_metrics_path: path to the old metrics CSV file (which save after `run_exp.py`).
        out_csv: path to save the new metrics CSV file.
        normalized: whether to normalize the predictions using train mean and std.

    """

    # fit_time & forecast_time
    ...
    for grp_id, grp_data in df_model.groupby("id"):
        ...
        if normalized:
            try:
                y_true = (y_true - stat_df.loc[grp_id, "mean"]) / (
...
def main(
    ...
    """Save metrics for each model prediction file. Save aggregated results to a CSV file.

    Args:
        results_dirs: list of directories with model predictions.
        normalized: whether to normalize the predictions using train mean and std.
